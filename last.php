	...
	if ( (isset($_SERVER['HTTP_REFERER'])?$_SERVER['HTTP_REFERER']:"") == "http://localhost/" ) {
		if ( $index_id = addslashes($_GET['id']) ) {
			...
			if ( $database_selected ) {
				...
				$open_query = mysqli_query($connection,"SELECT * FROM url_last WHERE id='$index_id'");
			...
			exit( header("location: http://log.earnmoneyclicking.com/?site=earnmoneyclicking&log=id_last") );
		...
		exit( header("location: http://log.earnmoneyclicking.com/?site=earnmoneyclicking&log=http_last") );
